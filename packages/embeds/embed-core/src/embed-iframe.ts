...
export const methods = {
  ui: function style(uiConfig: UiConfig) {
    ...
    if (stylesConfig) {
      console.warn(
        "Loop Customer Booking Embed: `styles` prop is deprecated. Use `cssVarsPerTheme` instead to achieve the same effect. Here is a list of CSS variables that are supported. https://github.com/calcom/cal.diy/blob/main/packages/config/tailwind-preset.js#L19"
      );
    }
...
function main() {
  ...
  if (top === window) {
    showPageAsNonEmbed();
    // We would want to avoid a situation where Loop Customer Booking embeds cal.com and then embed-iframe is in the top as well. In such case, we would want to avoid infinite loop of events being passed.
