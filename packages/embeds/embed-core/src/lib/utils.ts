...
export function buildConfigWithPrerenderRelatedFields({
  ...
  if (!backgroundSlotsFetch) {
    ...
    // Also, when used with Headless Router attributes setup, we might endup fetching slots for a lot of people, which would be a waste and unnecessary load on Loop Customer Booking resources
