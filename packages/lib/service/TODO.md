...
- [x] Unlocked attribute shouldn't get updates from SCIM but in case they still do, we follow this strategy
    ...
    - [x] The options created by SCIM aren't deletable from Loop Customer Booking UI. User can only remove options created by Loop Customer Booking i.e. where AttributeToUser.createdByDSyncId is null.
...
## Documentation
Okta stuff
- Removing the assignment of a user from the app, doesn't remove the user from Loop Customer Booking
